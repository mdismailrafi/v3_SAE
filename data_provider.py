...
def load_intraday(symbol: str, interval: str = "5m", lookback_days: int = 5, force_refresh: bool = False) -> pd.DataFrame:
    ...
    try:
        ...
        if not df.empty:
            ...
            status = {"provider": "Yahoo Finance", "source_type": "secondary intraday recovery", "rows": len(df), "csv_path": str(path), "live_error": " | ".join(errors)}
    ...
    if not cached.empty:
        status = {"provider": "Cached canonical CSV", "source_type": "automatic CSV recovery", "rows": len(cached), "live_error": " | ".join(errors)}
        cached.attrs["provider_status"] = status
        return cached

    raise RuntimeError("Intraday data unavailable from OpenChart/NSE and recovery sources. " f"Detail: {' | '.join(errors)}")


class YahooDailyProvider:
    """Primary daily/monthly OHLCV provider for Indian NSE symbols.

    Yahoo supports long historical periods and standard daily/monthly intervals;
    using it for OHLCV avoids spending Alpha Vantage quota on price history.
    """
    ...
    def _ticker(self, symbol: str):
        if yf is None:
            raise RuntimeError("yfinance is not installed")
        return yf.Ticker(f"{symbol.strip().upper()}.NS")

    def daily(self, symbol: str, period: str = "10y") -> pd.DataFrame:
        ticker = self._ticker(symbol)
        raw = ticker.history(period=period, interval="1d", auto_adjust=False, prepost=False, timeout=20)
        if raw is None or raw.empty:
            raise RuntimeError(f"Yahoo Finance returned no daily data for {symbol.upper()}.NS")
        df = _normalize_ohlcv(raw.reset_index())
        if df.empty:
            raise RuntimeError(f"Yahoo Finance returned no usable daily data for {symbol.upper()}.NS")
        self.last_status = {"provider": "Yahoo Finance", "source_type": "daily OHLCV", "rows": len(df), "ticker": f"{symbol.upper()}.NS"}
        return df

    def monthly(self, symbol: str, period: str = "10y") -> pd.DataFrame:
        ticker = self._ticker(symbol)
        raw = ticker.history(period=period, interval="1mo", auto_adjust=False, prepost=False, timeout=20)
        if raw is None or raw.empty:
            raise RuntimeError(f"Yahoo Finance returned no monthly data for {symbol.upper()}.NS")
        df = _normalize_ohlcv(raw.reset_index())
        if df.empty:
            raise RuntimeError(f"Yahoo Finance returned no usable monthly data for {symbol.upper()}.NS")
        self.last_status = {"provider": "Yahoo Finance", "source_type": "monthly OHLCV", "rows": len(df), "ticker": f"{symbol.upper()}.NS"}
        return df


class AlphaVantageProvider:
    """Fallback for daily/monthly OHLCV and primary provider for fundamentals."""

    def __init__(self, timeout: int = 20):
        self.key = _get_secret("ALPHAVANTAGE_API_KEY")
        self.timeout = timeout
        self.last_status: Dict[str, Any] = {"api_key_configured": bool(self.key)}

    def _query(self, params: Dict[str, Any], cache_key: str, ttl: int = 1800) -> Dict[str, Any]:
        if not self.key:
            raise RuntimeError("ALPHAVANTAGE_API_KEY is not configured in Streamlit Secrets/environment.")
        cached = _load_pickle("av_json", cache_key, ttl)
        if cached is not None:
            try:
                self.last_status.update({"source_type": "Alpha Vantage cache", "cache_key": cache_key})
        ...
        for i in range(3):
            try:
                ...
                if "Information" in data and not any(k in data for k in ["Time Series (Daily)", "Monthly Time Series", "Symbol"]):
                    raise RuntimeError(str(data["Information"]))
                pd.DataFrame([data]).to_pickle(_cache_path("av_json", cache_key))
                self.last_status.update({"source_type": "Alpha Vantage live", "cache_key": cache_key})
                return data
            except Exception as exc:
                last = exc
                if i < 2:
    ...
    def daily(self, symbol: str) -> pd.DataFrame:
        errors = []
        candidates = [f"{symbol.upper()}.BSE", f"NSE:{symbol.upper()}", symbol.upper()]
        for candidate in candidates:
            try:
                ...
                if series:
                    ...
                    out = _normalize_ohlcv(df.sort_index())
                    if not out.empty:
                        self.last_status.update({"provider": "Alpha Vantage", "ticker": candidate, "rows": len(out), "attempts": candidates})
                        return out
                errors.append(f"{candidate}: no Time Series (Daily)")
            except Exception as exc:
                errors.append(f"{candidate}: {exc}")
        self.last_status.update({"provider": "Alpha Vantage", "error": " | ".join(errors), "attempts": candidates})
        raise RuntimeError("Alpha Vantage daily fallback failed. " + " | ".join(errors))
            ...
            try:
                ...
                if series:
                    ...
                    if not out.empty:
                        self.last_status.update({"provider": "Alpha Vantage", "ticker": candidate, "rows": len(out), "attempts": candidates})
                        return out
                errors.append(f"{candidate}: no Monthly Time Series")
            except Exception as exc:
                errors.append(f"{candidate}: {exc}")
        self.last_status.update({"provider": "Alpha Vantage", "error": " | ".join(errors), "attempts": candidates})
        raise RuntimeError("Alpha Vantage monthly fallback failed. " + " | ".join(errors))
        ...
        for candidate in candidates:
            try:
                ...
                if data and "Symbol" in data:
                    self.last_status.update({"provider": "Alpha Vantage", "ticker": candidate})
                    return data
                errors.append(f"{candidate}: no Overview payload")
            except Exception as exc:
                errors.append(f"{candidate}: {exc}")
        self.last_status.update({"provider": "Alpha Vantage", "error": " | ".join(errors), "attempts": candidates})
        return {}
...
def load_daily(symbol: str) -> pd.DataFrame:
    """Daily pipeline: Yahoo Finance NSE -> Alpha Vantage fallback."""
    errors = []
    try:
        provider = YahooDailyProvider()
        df = provider.daily(symbol, period="10y")
        df.attrs["provider_status"] = dict(provider.last_status)
        return df
    except Exception as exc:
        errors.append(f"Yahoo Finance: {exc}")
    ...
    try:
        ...
        status = dict(provider.last_status)
        status["live_error"] = " | ".join(errors)
        df.attrs["provider_status"] = status
        return df
    except Exception as exc:
        errors.append(f"Alpha Vantage: {exc}")

    raise RuntimeError("No daily OHLCV data available. " + " | ".join(errors))


def load_monthly(symbol: str) -> pd.DataFrame:
    """Monthly pipeline: Yahoo Finance NSE -> Alpha Vantage fallback."""
    errors = []
    try:
        provider = YahooDailyProvider()
        df = provider.monthly(symbol, period="10y")
        df.attrs["provider_status"] = dict(provider.last_status)
        return df
    except Exception as exc:
        errors.append(f"Yahoo Finance: {exc}")
    ...
    raise RuntimeError("No monthly OHLCV data available. " + " | ".join(errors))


def load_overview(symbol: str) -> Dict[str, Any]:
    """Fundamentals remain Alpha Vantage based."""
