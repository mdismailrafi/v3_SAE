...
def add_indicators(df: pd.DataFrame, intraday: bool = False) -> pd.DataFrame:
    ...
    if intraday:
        # Session VWAP. Reset at each calendar day.
        tp = (h + l + c) / 3
        pv = tp * v
...
def technical_snapshot(x: pd.DataFrame, intraday: bool = False) -> Dict[str, object]:
    ...
    if pd.notna(last.get("ema20")):
        add("Price vs EMA20", "Above" if c > last.ema20 else "Below", 1 if c > last.ema20 else -1)
    if pd.notna(last.get("ema50")):
        add("Price vs EMA50", "Above" if c > last.ema50 else "Below", 1 if c > last.ema50 else -1)
    else: items["Price vs EMA50"] = "Unavailable"
    if pd.notna(last.get("ema200")):
        add("Price vs EMA200", "Above" if c > last.ema200 else "Below", 1 if c > last.ema200 else -1)
    ...
    if intraday and pd.notna(last.get("vwap")):
        add("VWAP", "Above" if c > last.vwap else "Below", 1 if c > last.vwap else -1)
...
def support_resistance(x: pd.DataFrame, window: int = 30) -> Tuple[float, float]:
    ...
    tail = x.tail(window)
    support = float(tail["low"].quantile(0.15))
    resistance = float(tail["high"].quantile(0.85))
...
def entry_plan(x: pd.DataFrame, horizon: str, intraday: bool = False) -> Dict[str, float | str]:
    ...
    if intraday:
        entry_low, entry_high = price - 0.20 * atr, price + 0.10 * atr
        stop = min(price - 1.1 * atr, support) if pd.notna(support) and support < price else price - 1.1 * atr
        risk = max(price - stop, 0.5 * atr)
        t1, t2 = price + 1.5 * risk, price + 2.2 * risk
    elif horizon == "Short-term":
        entry_low, entry_high = price - 0.5 * atr, price + 0.15 * atr
        stop = min(price - 1.5 * atr, support) if pd.notna(support) and support < price else price - 1.5 * atr
        risk = max(price-stop, 0.75*atr)
        t1, t2 = price + 1.5*risk, price + 2.5*risk
    else:
        entry_low, entry_high = price - 1.0 * atr, price + 0.25 * atr
        stop = price - 2.0 * atr
        risk = max(price-stop, atr)
        t1, t2 = price + 2.0*risk, price + 3.0*risk
...
def fundamental_snapshot(o: Dict[str, object]) -> Dict[str, object]:
    ...
    fields = {
        "MarketCap": "Market Capitalization", "PERatio": "P/E", "PriceToBookRatio": "P/B", "ReturnOnEquityTTM": "ROE", "ReturnOnAssetsTTM": "ROA",
        "ProfitMargin": "Profit Margin", "OperatingMarginTTM": "Operating Margin", "QuarterlyRevenueGrowthYOY": "Revenue growth YoY", "QuarterlyEarningsGrowthYOY": "Earnings growth YoY",
        "DebtToEquity": "Debt/Equity", "DividendYield": "Dividend yield", "Beta": "Beta", "BookValue": "Book value", "EPS": "EPS",
    }
    items = {}
    scores=[]
    for k, label in fields.items():
        v = _num(o, k)
        if pd.notna(v): items[label] = v; scores.append(v)
    # Transparent heuristic; not a peer-normalized fundamental model.
    points=[]
    for k, good_low, good_high in [("PERatio",0,25),("PriceToBookRatio",0,4),("DebtToEquity",0,1.5)]:
        v=_num(o,k)
        if pd.notna(v): points.append(1 if good_low < v <= good_high else -1)
...
def backtest_signal(x: pd.DataFrame, horizon: str, intraday: bool=False, hold_bars: int | None=None, cost_bps: float=10.0, slippage_bps: float=5.0) -> Dict[str, object]:
    ...
    # Avoid look-ahead: enter on next bar close and evaluate forward return.
    fwd=z["close"].shift(-bars)/z["close"]-1
    trades=fwd[signal & fwd.notna()].dropna()
    n=len(trades)
    ...
    # Evidence tiers deliberately require more than 20 observations.
    if n < 30: status="Exploratory"
    elif n < 100: status="Preliminary"
    elif n < 250: status="Moderate evidence"
    else: status="Stronger evidence"
...
def decision(tech: Dict[str, object], fund: Dict[str, object], horizon: str, backtest: Dict[str, object] | None = None, hard_risk: bool=False) -> Dict[str, object]:
    ts=tech.get("score",np.nan); fs=fund.get("score",np.nan)
    if horizon == "Long-term":
        score = fs if pd.isna(ts) else (0.35*ts + 0.65*fs if pd.notna(fs) else ts)
    elif horizon == "Short-term":
        score = ts if pd.isna(fs) else (0.70*ts + 0.30*fs if pd.notna(ts) else fs)
    else:
        score = ts
    # A demonstrated negative historical setup must not be overridden by a strong
    # point-in-time technical score. Small samples only affect confidence, not the
    # current signal itself.
    bt = backtest or {}
    n = int(bt.get("observations", 0) or 0)
    exp = bt.get("expectancy", np.nan)
    validation_penalty = False
    if horizon in {"Intraday", "Short-term"} and n >= 30 and pd.notna(exp) and float(exp) < 0:
        validation_penalty = True
        score = min(score, 57.9) if pd.notna(score) else score
...
def analyze(df: pd.DataFrame, horizon: str, intraday: bool=False, fundamentals: Dict[str, object] | None=None, interval: str="5m") -> Dict[str, object]:
    x=add_indicators(df,intraday=intraday)
    q=data_quality(x,interval,intraday)
    tech=technical_snapshot(x,intraday)
    fund=fundamental_snapshot(fundamentals or {})
    plan=entry_plan(x,horizon,intraday)
    bt=backtest_signal(x,horizon,intraday)
    hard_risk=False
    # Extreme leverage or negative profitability gets a penalty rather than being silently ignored.
    d=fundamentals or {}
    de=_num(d,"DebtToEquity"); pm=_num(d,"ProfitMargin")
    if pd.notna(de) and de>4: hard_risk=True
    if pd.notna(pm) and pm<0: hard_risk=True
    dec=decision(tech,fund,horizon,backtest=bt,hard_risk=hard_risk)
    return {"data":x,"quality":q,"technical":tech,"fundamental":fund,"plan":plan,"backtest":bt,"decision":dec,"candle_pattern":candlestick_pattern(x.iloc[-1]) if not x.empty else "—"}
